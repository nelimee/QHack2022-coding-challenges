...
"""In this challenge, we have implemented the Hadamard test, which compute the distance between quantum states embedding two data vectors.

Note: Only use one call to AmplitudeEmbedding as two separete calls results in an error.
...
def distance(A, B):
    ...
    def circuit(A,B):
        ...
        qml.AmplitudeEmbedding(features=np.kron(A,B), wires=[1,2], normalize=True)
        qml.Hadamard(wires=0)
        qml.CSWAP(wires=[0,1,2])
        qml.Hadamard(wires=0)
        return qml.probs(0)

    probs0 = circuit(A,B)[0]
    dotprodAB = np.sqrt(np.abs(2.0*(probs0 - 0.5)))
    distance_AB = np.sqrt(2.0*(1.0-dotprodAB))
